	...
	return fetch(`https://api.direkt.bahn.guru/${formatStationId(origin.id)}`)
		...
		.then(async results => {
			...
			if (resultsWithLocations.length === 0) {
				Sweetalert.fire('Hmm…', 'Leider konnten für die Stadt, die du gesucht hast, keine Verbindungen gefunden werden.', 'warning')
			}
		})
}
...
const isLongDistance = s => {
	return s.products && (s.products.nationalExp || s.products.national) && isUicLocationCode(formatStationId(s.id))
}
...
const geocoder = new MapboxGeocoder({
	geocode: async (query) => {
		...
		const filteredResults = results.filter(x => isLongDistance(x) && !isRegion(x) && hasLocation(x))
