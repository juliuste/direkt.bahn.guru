...
const stationById = id => {
	return stations.find(s => (formatStationId(s.id) === formatStationId(id)) && formatStationId(id))
}
	...
	return fetch(`https://api.direkt.bahn.guru/${formatStationId(origin.id)}`)
		...
		.then(async results => {
			const resultsWithLocations = results.map(r => ({
				...r,
				location: r.location || locationById(r.id)
			})).filter(r => !!r.location)
			...
			map.on('click', 'stations', e => {
				...
				if (link) window.open(link, 'bahn-guru')
			})
...
const hasLocation = s => {
	const location = locationById(s.id)
	return !!location
}
...
map.on('load', () => {
	document.querySelector('.mapboxgl-ctrl-attrib').innerHTML = '<b><a href="https://github.com/juliuste/direkt.bahn.guru">GitHub</a></b> ' + document.querySelector('.mapboxgl-ctrl-attrib').innerHTML
